...
public:
  ...
  std::string to_string() const { return m_has_value ? std::to_string(m_value) : "null"; }
...
bool any_is_null(T t, Ts ... ts)
{
  return !t.has_value() || any_is_null(ts ...);
}

template<typename T>
bool any_is_null(T t) { return !t.has_value(); }
...
RT applyfuck(RT (*fn)(Ts...), Ts...ts) { return apply(std::function<(Ts...)>(fn), ts ...); }
...
int main() {
  ...
  cout << "a + b = " << apply(std::function<int(int,int)>(std::plus<int>()), 3, 8) << endl;
