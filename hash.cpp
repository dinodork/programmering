...
template <typename T> int getHashCode(const T &t) { return t->getHashCode(); }

template <typename T, int (*pekareTillGetHashCode)(const T)> class HaschKaka {

  void haschibaschi(const T &t) { cout << "hash " << pekareTillGetHashCode(t); }
};
