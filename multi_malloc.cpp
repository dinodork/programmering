...
template <typename... CtorArgs> Apa *makeAnApa(CtorArgs... ctorArgs) {
  return new Apa(ctorArgs...);
}

template <typename T, typename... CtorArgs> Apa *makeAn(CtorArgs... ctorArgs) {
  return new T(ctorArgs...);
}

template <typename... Ts> class Type_holder {};
...
template <typename T, typename... Ts> struct add_pointers {};
