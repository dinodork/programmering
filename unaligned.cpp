...
template <typename T1, typename T2> T1 read_unaligned(T2 address) {
  T1 t;
  memcpy(&t, address, sizeof(T1));
  return t;
}

template <> char *read_unaligned<char *, char *>(char *address);

template <typename T1, typename T2> void write_unaligned(T1 value, T2 address) {
