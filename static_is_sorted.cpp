...
template <size_t N, const int(Arr)[N], size_t I = N - 1> struct is_sorted {
  static constexpr is_sorted<N, Arr, I - 1> arr{};
  static constexpr bool value = Arr[I] > Arr[I - 1] && arr.value;
};

template <size_t N, const int(Arr)[N]> struct is_sorted<N, Arr, 1> {
  static constexpr bool value = true;
};

template <const int(Arr)[]> struct is_sorted_som_Fan {
  static constexpr is_sorted<sizeof(Arr) / sizeof(Arr[0]), Arr,
